...
namespace AutoRSN.Repositories
{
    ...
    {
        public List<Report> getReport(string dateFrom, string dateTo, string user , string realname , string customergroup , string option)
        {
            ...
            if (option.Equals("generatedon"))
            {
                if (customergroup != "")
                {
                    cmd.CommandText = $"select a.matnr,a.kdmat,a.arktx,b.servicetype,b.billto,'True' as CHARGE,b.ex_factory,b.fca_fob,b.door_to_door, " +
                                        ...
                                        "END as FORWARDER, " +
                                        "a.shipshift,(nvl(a.shipqty, 0) * nvl(a.netpr, 0)) as AMOUNT,a.createdby,b.customergroup,a.SHIPSTATUS,a.SHIPSTATUSON,a.GENERATEDON " +
                                          $"from z_t_rsn_final a , rsn_st_customer b  where a.kunnr1 = b.customercode and a.kunnr1 in (select customercode from rsn_st_customer where customergroup  = '{customergroup}') " +
                                            $"and a.GENERATEDON between to_date('{dateFrom} 00:00:00', 'mm/dd/yyyy hh24: mi:ss') and to_date('{dateTo} 23:59:00', 'mm/dd/yyyy hh24: mi:ss') and a.shipdate is not null and a.edatu is not null";
                }
                else
                {
            ...
            if (option.Equals("shipon"))
            {
                if (customergroup != "")
                {
                    cmd.CommandText = $"select a.matnr,a.kdmat,a.arktx,b.servicetype,b.billto,'True' as CHARGE,b.ex_factory,b.fca_fob,b.door_to_door, " +
                                        ...
                                        "a.shipshift,(nvl(a.shipqty, 0) * nvl(a.netpr, 0)) as AMOUNT,a.createdby,b.customergroup,a.SHIPSTATUS,a.SHIPSTATUSON,a.GENERATEDON " +
                                          $"from z_t_rsn_final a , rsn_st_customer b  where a.kunnr1 = b.customercode and a.kunnr1 in (select customercode from rsn_st_customer where customergroup  = '{customergroup}') " +
                                            $"and to_date(a.shipdate , 'dd/mm/yyyy') between to_date('{dateFrom} 00:00:00', 'mm/dd/yyyy hh24: mi:ss') and to_date('{dateTo} 23:59:00', 'mm/dd/yyyy hh24: mi:ss') and a.shipdate is not null and a.edatu is not null";
                }
                else
                {
            ...
            Debug.WriteLine(cmd.CommandText);
             ...
             while(dr.Read())
                ...
                if(dr.GetValue(14) != null)
                {
                  ...
                  //  catch(Exception ex)
                  //  {
                      //  System.Diagnostics.Debug.WriteLine(ex.Message + " == " + dr.GetValue(14));
                ...
                report.SHIPSTATUS = dr.GetValue(31).ToString();
                report.SHIPSTATUSON = dr.GetValue(32).ToString();
                report.DATE_TIME = dr.GetValue(33).ToString();
